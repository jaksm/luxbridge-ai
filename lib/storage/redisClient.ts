...
import { PlatformAsset, PlatformType, UserPortfolioHolding, PlatformInfo } from "../types/platformAsset";
import { GetAssetParams, GetAssetsByPlatformParams, GetUserPortfolioParams } from "../types/schemas";
...
export class AssetStorage {
  ...
  async getAssetsByPlatform(params: GetAssetsByPlatformParams): Promise<PlatformAsset[]> {
    await ensureConnected();
  ...
  async storeAsset(asset: PlatformAsset, platform: PlatformType): Promise<void> {
    await ensureConnected();
    const key = `platform:${platform}:assets:${asset.assetId}`;
    await redis.set(key, JSON.stringify(asset));
  }

  async getUserPortfolio(params: GetUserPortfolioParams): Promise<UserPortfolioHolding[]> {
    await ensureConnected();
  ...
  async storeUserPortfolio(userId: string, platform: PlatformType, holdings: UserPortfolioHolding[]): Promise<void> {
    await ensureConnected();
  ...
  async getAssetsByIds(assetIds: string[], platform: PlatformType): Promise<PlatformAsset[]> {
    await ensureConnected();
