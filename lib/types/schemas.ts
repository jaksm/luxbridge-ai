...
export const GetAssetSchema = z.object({
  ...
  assetId: z.string()
    .describe("Unique asset identifier within the platform")
}).describe("Parameters for retrieving a specific asset from a platform");

export const GetAssetsByPlatformSchema = z.object({
  platform: z.enum(["splint_invest", "masterworks", "realt"])
...
export const GetUserPortfolioSchema = z.object({
  ...
  userId: z.string()
    .describe("Unique user identifier for portfolio lookup")
}).describe("Parameters for retrieving user portfolio holdings from a specific platform");

export const SemanticSearchSchema = z.object({
  query: z.string()
    .describe("Natural language search query for finding relevant assets"),
  platform: z.enum(["splint_invest", "masterworks", "realt"]).optional()
...
}).describe("Parameters for semantic search across RWA assets using natural language queries");
...
export const AssetQuerySchema = z.object({
  ...
  minValue: z.number().optional()
    .describe("Minimum asset value"),
  maxValue: z.number().optional()
    .describe("Maximum asset value"),
  riskCategory: z.enum(["conservative", "moderate", "aggressive", "speculative"]).optional()
    .describe("Risk category filter")
});

export type GetAssetParams = z.infer<typeof GetAssetSchema>;
export type GetAssetsByPlatformParams = z.infer<typeof GetAssetsByPlatformSchema>;
