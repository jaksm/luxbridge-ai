...
import { generateJWT, validateJWT } from "./jwtUtils";
...
export async function validatePlatformCredentials(
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/${platform}/auth/login`, {
      method: 'POST',
    ...
    return {
      ...
      expiresAt: authData.expiresIn ? Date.now() + (authData.expiresIn * 1000) : undefined
    };
...
export async function makeAuthenticatedPlatformCall(
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/${platform}${endpoint}`, {
    ...options,
    headers: {
      ...options.headers,
...
export async function getAllUserPlatformLinks(luxUserId: string): Promise<PlatformLink[]> {
...
export async function validateAllPlatformLinks(luxUserId: string): Promise<void> {
  const platforms: PlatformType[] = ['splint_invest', 'masterworks', 'realt'];
  ...
  for (const platform of platforms) {
    ...
    if (link) {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/${platform}/auth/me`, {
          headers: {
