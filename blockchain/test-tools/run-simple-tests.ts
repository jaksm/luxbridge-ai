import {
  SimpleToolTester,
  type SimpleToolTestResult,
} from "./simple-tool-test";
...
export async function runSimpleToolTests(
  ...
  try {
    ...
    const tokenizeWineResult = await tester.testTokenizeAsset(
      "splint_invest",
      "WINE-TEST-001",
    );
    ...
    const tokenizeArtResult = await tester.testTokenizeAsset(
      "masterworks",
      "ART-TEST-001",
    );
    ...
    const metadataWineResult = await tester.testGetAssetMetadata(
      "splint_invest",
      "WINE-TEST-001",
    );
    ...
    const metadataArtResult = await tester.testGetAssetMetadata(
      "masterworks",
      "ART-TEST-001",
    );
    ...
    const metadataNonExistentResult = await tester.testGetAssetMetadata(
      "splint_invest",
      "NON-EXISTENT-001",
    );
...
function printOverallSummary(results: SimpleToolTestResult[]): void {
  ...
  console.log(
    `${successCount === totalCount ? "✅" : "❌"} TOTAL: ${successCount}/${totalCount} passed`,
  );
...
if (require.main === module) {
  ...
  runSimpleToolTests(verbose)
    .then((results) => {
      ...
      const criticalSuccessCount = criticalTests.filter(
        (r) => r.success,
      ).length;
