...
async function debugContracts() {
  ...
  try {
    // Test oracle state
    console.log("\n🔮 Oracle State:");
    const [price1, timestamp1] = await sdk.oracle.getPrice("splint_invest", "BORDEAUX-2019");
    console.log(`  splint_invest:BORDEAUX-2019 = ${ethers.formatEther(price1)} @ ${timestamp1}`);
  } catch (error: any) {
    ...
    try {
      await sdk.oracle.mockPriceUpdate("splint_invest", "BORDEAUX-2019", ethers.parseEther("100000"));
      console.log("  ✅ Price set successfully");
      
      const [price2, timestamp2] = await sdk.oracle.getPrice("splint_invest", "BORDEAUX-2019");
      console.log(`  After update: ${ethers.formatEther(price2)} @ ${timestamp2}`);
    } catch (err: any) {
