...
async function runBasicBlockchainTest(): Promise<void> {
  ...
  try {
    ...
    // Register platforms
    const platformTx1 = await factory.registerPlatform(
      "splint_invest",
      "https://api.splint.com",
    );
    await platformTx1.wait();
    const platformTx2 = await factory.registerPlatform(
      "masterworks",
      "https://api.masterworks.com",
    );
    await platformTx2.wait();
    const platformTx3 = await factory.registerPlatform(
      "realt",
      "https://api.realt.com",
    );
    ...
    try {
      const platformInfo = await sdk.getPlatformInfo({
      ...
      console.log(
        `❌ Platform info failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    try {
      ...
      try {
        ...
        console.log(
          `❌ Asset metadata failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
      ...
      try {
        ...
        console.log(
          `❌ Price update failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
      ...
      try {
        ...
        console.log(
          `❌ Get price failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
      ...
      try {
        ...
        try {
          ...
          console.log(
            `❌ Add liquidity failed: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
        ...
        try {
          ...
          console.log(
            `❌ Get amount out failed: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      } catch (error) {
        results.push({
      ...
      console.log(
        `❌ Asset tokenization failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    console.log("================================");
    console.log(
      "This test verifies the core blockchain functionality that powers ALL MCP tools:",
    );
    ...
    const toolsMap = {
      ...
      mock_price_update: [
        "registerCalculateArbitrageOpportunityTool (pricing component)",
      ],
    ...
    console.log(
      "  - registerDelegateTradingPermissionsTool → Automation contract",
    );
    ...
    console.log(
      "  - registerCalculateArbitrageOpportunityTool → Oracle pricing",
    );
