...
async function focusedOracleTest() {
  ...
  try {
    ...
    try {
      const [price, timestamp] = await sdk.oracle.getPrice("splint_invest", "BORDEAUX-2019");
      console.log(`✅ Direct oracle call: ${ethers.formatEther(price)} at ${timestamp}`);
  ...
  try {
    ...
    console.log(`Oracle contract address: ${await oracleContract.getAddress()}`);
    
    // Try to read the platformAssets mapping directly
    const assetData = await oracleContract.platformAssets("splint_invest", "BORDEAUX-2019");
