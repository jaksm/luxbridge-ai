...
describe("Cross-Platform Trade Integration", function () {
  ...
  async function setupAssetsFixture() {
    ...
    const wineTokenAddress = await factory.getTokenAddress("splint_invest", "BORDEAUX-2019");
    const artTokenAddress = await factory.getTokenAddress("masterworks", "PICASSO-042");
  ...
  describe("Complete Cross-Platform Trading Flow", function () {
    ...
    it("Should complete a full cross-platform trade: Wine → Art", async function () {
      ...
      await oracle.mockPriceUpdate("splint_invest", "BORDEAUX-2019", ethers.parseEther("120000"));
      await oracle.mockPriceUpdate("masterworks", "PICASSO-042", ethers.parseEther("250000"));
      ...
      await wineToken.connect(alice).approve(await amm.getAddress(), aliceWineAmount);
      await artToken.connect(alice).approve(await amm.getAddress(), aliceArtAmount);
      ...
      await wineToken.connect(alice).transfer(charlie.address, charlieWineAmount);
      ...
      await wineToken.connect(charlie).approve(await amm.getAddress(), charlieWineAmount);
    ...
    it("Should detect and exploit arbitrage opportunities", async function () {
      ...
      await wineToken.connect(alice).approve(await amm.getAddress(), wineAmount1);
      ...
      await oracle.mockPriceUpdate("splint_invest", "BORDEAUX-2019", ethers.parseEther("150000"));
      await oracle.mockPriceUpdate("masterworks", "PICASSO-042", ethers.parseEther("150000"));
    ...
    it.skip("Should handle automated trading via AI agent", async function () {
      ...
      await automation.connect(charlie).delegateTrading(
        maxTradeSize,
        dailyLimit,
        allowedAssets
      );
      ...
      await wineToken.connect(alice).transfer(charlie.address, ethers.parseEther("10000"));

      // Charlie approves automation contract to spend his tokens
      await wineToken.connect(charlie).approve(
      ...
      const tx = await automation.connect(aiAgent).queueAutomatedTrade(
        charlie.address,
      ...
      const permissions = await automation.getTradingPermission(charlie.address);
      expect(permissions.dailySpent).to.equal(tradeAmount);
    });

    it("Should prevent unauthorized trades in automation", async function () {
      const { automation, alice, charlie } = await loadFixture(setupAssetsFixture);

      // Try to queue trade without delegation
      await expect(
        automation.connect(alice).queueAutomatedTrade(
          charlie.address,
          "BORDEAUX-2019",
          "PICASSO-042",
    ...
    it("Should enforce daily limits in automated trading", async function () {
      const { automation, aiAgent, charlie, alice, wineToken } = await loadFixture(setupAssetsFixture);
      ...
      // Give Charlie tokens
      await wineToken.connect(alice).transfer(charlie.address, ethers.parseEther("5000"));
      await wineToken.connect(charlie).approve(await automation.getAddress(), ethers.parseEther("5000"));
      ...
      await expect(
        automation.connect(aiAgent).queueAutomatedTrade(
          charlie.address,
          "BORDEAUX-2019",
          "PICASSO-042",
          tradeAmount,
    ...
    it("Should handle asset burning when sold on platform", async function () {
      const { factory, alice, wineToken } = await loadFixture(setupAssetsFixture);
      ...
      await wineToken.connect(alice).approve(await factory.getAddress(), burnAmount);
      
      await factory.connect(alice).burnTokens(
        "splint_invest",
        "BORDEAUX-2019",
        burnAmount
      );
    ...
    it("Should update platform statistics correctly", async function () {
      const { factory, oracle, alice, owner } = await loadFixture(setupAssetsFixture);

      // Check initial platform stats
      const initialPlatformInfo = await factory.getPlatformInfo("splint_invest");
      expect(initialPlatformInfo.totalAssetsTokenized).to.equal(1);
      expect(initialPlatformInfo.totalValueLocked).to.equal(ethers.parseEther("100000"));
      ...
      await factory.connect(oracleSigner).updateValuation(
        "splint_invest",
        "BORDEAUX-2019",
      ...
      const updatedPlatformInfo = await factory.getPlatformInfo("splint_invest");
      expect(updatedPlatformInfo.totalValueLocked).to.equal(ethers.parseEther("150000"));
    });
  });

  describe("Gas Optimization Tests", function () {
    it("Should efficiently batch operations", async function () {
      const { factory, alice } = await loadFixture(deployLuxBridgeSystemFixture);
      ...
      const tx1 = await factory.connect(alice).tokenizeAsset(
        platforms[0], assetIds[0], supplies[0], types[0], hashes[0], valuations[0]
      );
  ...
  describe("Error Handling and Edge Cases", function () {
    it("Should handle zero amount swaps gracefully", async function () {
      const { amm, wineToken, artToken } = await loadFixture(setupAssetsFixture);
    ...
    it("Should handle expired automated trades", async function () {
      const { automation, aiAgent, charlie } = await loadFixture(setupAssetsFixture);
      ...
      const tradeId = await automation.connect(aiAgent).queueAutomatedTrade.staticCall(
        charlie.address,
        "BORDEAUX-2019",
        "PICASSO-042",
        ethers.parseEther("1000"),
      ...
      await automation.connect(aiAgent).queueAutomatedTrade(
        charlie.address,
        "BORDEAUX-2019",
        "PICASSO-042",
        ethers.parseEther("1000"),
