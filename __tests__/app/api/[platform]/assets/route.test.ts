...
vi.mock("@/lib/storage/redisClient");
  ...
  describe("GET", () => {
    ...
    it("should handle semantic search", async () => {
      const { assetStorage } = await import("@/lib/storage/redisClient");
      const { SemanticAssetSearch } = await import(
        "@/lib/utils/semanticSearch"
      );

      const mockSearcher = {
        searchAssets: vi
          .fn()
          .mockResolvedValue(["WINE-BORDEAUX-001", "WINE-BURGUNDY-002"]),
      };
      vi.mocked(SemanticAssetSearch).mockImplementation(
        () => mockSearcher as any,
      );
      vi.mocked(assetStorage.getAssetsByIds).mockResolvedValue(
        mockAssets.splint_invest.slice(0, 2),
      );
      const request = createMockRequest("splint_invest", {
        semanticSearch: "fine wine",
      });
      const context = createMockContext({ platform: "splint_invest" });
      const response = await GET(request, context);
        query: "fine wine",
    ...
    it("should apply category filter", async () => {
      ...
      const wineAssets = mockAssets.splint_invest.filter(
        (asset) => asset.category === "wine",
      );
      vi.mocked(assetStorage.getAssetsByPlatform).mockResolvedValue(
        mockAssets.splint_invest,
      );
    ...
    it("should respect limit parameter", async () => {
      ...
      const response = await GET(request, context);
      const data = await expectJSONResponse(response, 200);
      expect(data.assets).toHaveLength(
        Math.min(2, mockAssets.splint_invest.length),
      );
    });
    it("should enforce maximum limit", async () => {
      const { assetStorage } = await import("@/lib/storage/redisClient");
    ...
    it("should handle storage errors", async () => {
      ...
      vi.mocked(assetStorage.getAssetsByPlatform).mockRejectedValue(
