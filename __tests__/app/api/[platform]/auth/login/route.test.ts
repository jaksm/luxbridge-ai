...
import { createMockRequest, createMockContext, expectJSONResponse, expectErrorResponse } from "@/__tests__/utils/testHelpers";
import { mockLoginRequests, createMockLoginResponse } from "@/__tests__/fixtures/mockUsers";
...
describe("Platform Auth Login Route", () => {
  ...
  describe("POST", () => {
    it("should login successfully with valid credentials", async () => {
      ...
      expect(validateCredentials).toHaveBeenCalledWith("test@example.com", "password123");
    ...
    it("should reject invalid credentials", async () => {
      ...
      await expectErrorResponse(response, 401, "invalid_credentials", "Invalid email or password");
    });
    ...
    it("should validate required fields", async () => {
      ...
      for (const { body } of testCases) {
        ...
        await expectErrorResponse(response, 400, "missing_credentials", "Email and password are required");
      }
    });
    ...
    it("should handle authentication service errors", async () => {
      ...
      vi.mocked(validateCredentials).mockRejectedValue(new Error("Database error"));
