...
export const createMockRequest = (body?: any, headers?: Record<string, string>): NextRequest => {
...
export const createMockRequestWithPlatform = (
  ...
  const headers: Record<string, string> = authToken ? { authorization: createMockBearerToken(authToken) } : {};
...
export const createMockContext = <T extends Record<string, any>>(params: T) => ({
  params: Promise.resolve(params)
});

export const expectJSONResponse = async (response: Response, expectedStatus: number, expectedData?: any) => {
...
export const mockEnvironmentVariables = (overrides: Record<string, string> = {}) => {
...
export const platforms: PlatformType[] = ["splint_invest", "masterworks", "realt"];
