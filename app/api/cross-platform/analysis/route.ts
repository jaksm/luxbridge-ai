...
import { constructUserPortfolio, calculateDiversificationScore, calculateLiquidityScore, calculateRiskScore } from "@/lib/utils/portfolioCalculator";
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const platform of platforms) {
      ...
      if (holdings.length > 0) {
        try {
          ...
          totalValue += constructedAssets.reduce((sum, asset) => sum + asset.currentValue, 0);
        } catch (error) {
    ...
    const diversificationScore = allAssets.length > 0 ? calculateDiversificationScore(allAssets) : 0;
    const liquidityScore = allAssets.length > 0 ? calculateLiquidityScore(allAssets) : 0;
    const crossPlatformRisk = allAssets.length > 0 ? calculateRiskScore(allAssets) : 0;
    ...
    return NextResponse.json({
      ...
      rebalanceRecommendations: allAssets.length === 0 ? [
        {
          action: "buy" as const,
