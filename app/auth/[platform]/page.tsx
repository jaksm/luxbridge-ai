...
const PLATFORM_CONFIG = {
  'splint-invest': {
    name: 'Splint Invest',
    description: 'Connect your Splint Invest account to access wine and luxury asset investments',
    color: 'from-purple-600 to-purple-800',
    category: 'Alternative Assets'
  },
  'masterworks': {
    name: 'Masterworks',
    description: 'Link your Masterworks account to include art investments in your portfolio',
...
export default function PlatformAuthPage({ params }: AuthPageProps) {
  ...
  useEffect(() => {
    if (!sessionId) {
      setError('Invalid session. Please restart the linking process from your MCP client.');
    }
  }, [sessionId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/auth/platforms/${params.platform}/complete`, {
        method: 'POST',
      ...
      if (result.success) {
        ...
        router.push(`/auth/${params.platform}/complete?status=success&session=${sessionId}`);
      } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background to-secondary/20 p-4">
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            <p>No valid session found. Please start the authentication process from your MCP client.</p>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-background to-secondary/20 flex items-center justify-center p-4">
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="text-center space-y-4">
          <div className={`w-16 h-16 mx-auto rounded-full bg-gradient-to-r ${platformConfig.color} flex items-center justify-center shadow-lg`}>
            <ExternalLink className="w-8 h-8 text-white" />
          </div>
          <div>
            <CardTitle className="text-2xl font-bold">Connect {platformConfig.name}</CardTitle>
