...
async function main() {
  ...
  if (!FACTORY_ADDRESS) {
    console.log(
      "❌ Missing contract addresses. Please set environment variables:",
    );
  ...
  try {
    ...
    // Check pool creation
    const wineToken = await factory.getTokenAddress(
      "splint_invest",
      "BORDEAUX-2019",
    );
    const artToken = await factory.getTokenAddress(
      "masterworks",
      "PICASSO-042",
    );
    ...
    try {
      ...
      console.log(
        `   ✅ Pool reserves: ${ethers.formatEther(poolInfo.reserveA)} / ${ethers.formatEther(poolInfo.reserveB)}`,
      );
    } catch (e) {
    ...
    try {
      ...
      console.log(
        `   ✅ Wine price: ${ethers.formatEther(price)} ETH (updated: ${new Date(Number(timestamp) * 1000).toLocaleString()})`,
      );
    } catch (e) {
