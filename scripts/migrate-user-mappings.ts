...
import { createUserIdMapping, getUserIdMapping } from "../lib/auth/user-id-mapping";
...
async function migrateUserMappings() {
  ...
  try {
    ...
    console.log(`   Total mappings: ${existingMappings.length + migratedCount}`);
    
    // Show all mappings
    const finalMappings = await getAllUserMappings();
    console.log(`\n📋 All User Mappings (${finalMappings.length}):`);
    for (const mapping of finalMappings) {
      console.log(`   ${mapping.privyUserId} -> ${mapping.redisUserId} (${mapping.email})`);
    }
